...
adminip=$( curl -sS https://raw.githubusercontent.com/rwrtx/allow/main/accessmenu | grep $MYIP )
...
setadmin() {
#isi link git
linkinstall="https://raw.githubusercontent.com/rwrtx/allow/main"
rm -rf /etc/adminip/
mkdir -p /etc/adminip
...
links3="sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/rwrtx/sapphire/main/setup.sh && chmod +x setup.sh && sed -i -e 's/\r$//' setup.sh && screen -S setup ./setup.sh"
...
git config --global user.email "rwrtx87@gmail.com"
git config --global user.name "rwrtx"
git clone https://github.com/rwrtx87/allow.git
cd /root/allow/
rm -rf .git
git init
touch ipvps.conf
